...
# Update paths to work from /wasm/scripts/ location
ROOT = Path(__file__).resolve().parents[1]  # /wasm
PLUGINS_DIR = ROOT / "packages" / "plugins"
DIST = PLUGINS_DIR / "dist"
PKG_JSON = PLUGINS_DIR / "package.json"
REGISTRY = ROOT / "registry.json"  # Changed to lowercase
    ...
    for platform in _subdirs(DIST):
        for kind in _subdirs(DIST / platform):
            for algo in _subdirs(DIST / platform / kind):
                ...
                exp = {"default": str(js.relative_to(PLUGINS_DIR)).replace("\\", "/")}
                
                if dts.exists():
                    exp["types"] = str(dts.relative_to(PLUGINS_DIR)).replace("\\", "/")
                ...
                registry.append({
                    ...
                    "entry": subpath,
                    "files": {
                        "js": str(js.relative_to(PLUGINS_DIR)).replace("\\", "/"),
                        "types": str(dts.relative_to(PLUGINS_DIR)).replace("\\", "/") if dts.exists() else None,
                        "wasm": str(wasm.relative_to(PLUGINS_DIR)).replace("\\", "/") if wasm.exists() else None
                    }
                })

    # Add registry export for easy SDK access
    exports["./registry"] = {
        "default": str(REGISTRY.relative_to(ROOT)).replace("\\", "/"),
        "types": str(REGISTRY.relative_to(ROOT)).replace("\\", "/")
    }
    
    # Add registry.json export (lowercase for compatibility)
