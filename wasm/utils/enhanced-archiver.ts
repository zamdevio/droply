// Enhanced archiver that embeds metadata inside compressed/archived files
import { getArchiver } from './archiver';
import { getCompressor } from './compressor';
import type { ArchiveAlgo, CompressionAlgo, ProcessMetadata } from './types';

// Metadata file name that gets embedded in archives
const METADATA_FILENAME = '.__droply_meta.json';
...
export async function createMetadataFile(
  ...
  try {
    // Create metadata filename: .droply_meta.json
 ...
 * @deprecated This approach corrupts files. Use separate metadata files instead.
 */
